...
namespace JSI.AppObject
{
    public class JSIAppRect3D : JSIAppGeom3D
    {
        ...
        public void setSize(float width, float height)
        {
            ...
            this.mGeom = new JSIRect3D(width, height, rect.getPos(), rect.getRot());
        ...
        // constructor
        public JSIAppRect3D(string name, float width, float height, Color color) : base($"{name}/Rect3D")
        {
            this.mGeom = new JSIRect3D(width, height, Vector3.zero, Quaternion.identity);
            this.mColor = color;
            this.refreshAtGeomChange();
        }
